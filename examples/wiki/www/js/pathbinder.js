(function($) {
  ...
  function triggerOnPath(path) {
    path = path.replace(/^#/,'');
    // console.log("triggerOnPath", path);
    ...
    for (var i=0; i < $.pathbinder.paths.length; i++) {
      ...
      if ((path_params = pathSpec.matcher.exec(path)) !== null) {
        ...
        // console.log("pathSpec", pathSpec, params);
  ...
  $.fn.bindPath = function (path, fun) {
  	// console.log('bind', path);
  	var self = $(this);
    self.bind(path, fun);
    self.pathbinder(path, path);
    return this;
  };
    ...
    $.each(pathList, function() {
      ...
      if (path) {
        ...
        var pathSpec = makePathSpec(path, function(params) {
          ...
          // console.log("trigger path: "+path+" params: ", name, params);
